...
in nucleons, and most nucleon mass is dynamical QCD energy: these are two distinct
statements. Neither assigns a universal percentage of matter's mass to the
geometric flux-tube picture.\refn{3}

\openingfigure{figN0_atom_to_quarks}{Inside a helium atom, then a proton.
The separate scale windows show constituent bookkeeping, not classical particle
trajectories or a calculated density. Colored regions are schematic; the proton's
$uud$ label specifies net valence content, not its entire quantum state.
Original diagram; physical interpretation follows sources 1--3.}{fig:n0}

\heading{0.2}{Confinement, and the boundary of the explanation}

QCD has an unusual scale dependence. At sufficiently high momentum transfer,
which probes short distances, its effective coupling becomes small. This is
\emph{asymptotic freedom}. It does not mean that a quark can leave a proton for
free: short-distance scattering and separating a quark into an isolated outgoing
particle are different questions. At longer distances, the weak-coupling
expansion ceases to be reliable. Extrapolating that expansion is not a proof of
confinement.\refn{4}

\emph{Color confinement} is the absence of isolated colored particles in the
observable outgoing states of ordinary low-energy experiments. A useful picture
comes from slowly separating a very heavy quark and antiquark. Over an
intermediate range, their color-field energy concentrates in an elongated region,
or \emph{flux tube}. In its approximately linear regime,
\[
V(r)\simeq V_0+\mathcal T_s r,
\qquad F_r=-\frac{\dd V}{\dd r}\simeq-\mathcal T_s.
\]
Here $r$ is separation, $V_0$ an irrelevant reference energy, and $\mathcal T_s$
the string tension, with units of energy per length, hence force. More separation
requires more work; the attraction need not grow without bound. With dynamical
quarks, sufficient energy can instead form an additional quark--antiquark pair:
the string breaks into color-neutral hadrons, not free quarks. Lattice calculations
resolve this process. This static-source picture is not a literal photograph of
three rigid strings inside a moving proton.\refn{5}

\panel{An open mathematical problem, not an absence of physics}{The Yang--Mills
...
\]
No physical excitation can have an energy strictly between zero and $\Delta$.
The official problem concerns pure Yang--Mills theory; confinement in QCD with
quarks is a related but distinct question. Neither follows just by quoting
asymptotic freedom.\refn{6}}
