...
%   (or: cd thesis && latexmk -pdf thesis-draft.tex)
% =====================================================================
...
\hypersetup{
  pdftitle={Let the Machine Reveal Itself: From Fusion Physics to the Spherical-Tokamak Centre Stack (working thesis draft)},
  pdfauthor={Jony (TUNEM)},
  pdfsubject={Master's thesis working draft, Tsinghua Nuclear Engineering and Management},
  pdfkeywords={tokamak, spherical tokamak, fusion, centre stack, Freidberg, TUNEM}}
...
\newcommand{\freidbergvariable}[2]{%
  \begin{center}\fcolorbox{blue!60!black}{blue!5}{\parbox{0.90\textwidth}{%
  \textbf{Primary Freidberg parameter --- #1.} #2}}\end{center}}
...
\freidbergvariable{$a$ (minor radius) and $R_0$ (major radius)}{Closing the
...
% ============================== PAGE 11 ==============================
\section*{11. The transformer has a clock}
...
\freidbergvariable{$\kappa$ (elongation)}{Equation~\eqref{eq:kappa} changes the plasma
...
\freidbergvariable{$q_*$ and $I_p$}{Equation~\eqref{eq:qstar} is the shaped global
...
\freidbergvariable{$B_0$ and $\beta$}{Equation~\eqref{eq:pfus-scaling}
...
$2\pi^2R_0a^2\kappa$ recur throughout Freidberg's design closure. The equation is a
...
minimum state carried forward into the Freidberg-style closure problem. A proposed
...
\freidbergvariable{$T$, $n$, and $p$}{For the equal 50--50 mixture used here,
...
\freidbergvariable{$\tau_E$}{Energy-confinement time is the variable that converts
the plasma state $(n,T,V)$ into an energy-loss power. It is a core Freidberg input,
...
Freidberg power ledger but cannot by itself establish net electricity.}
...
assumption behind an empirical $\tau_E$ explicit. Freidberg-style closure cannot
...
opening. When a particular confinement regression is introduced in the Freidberg
...
\freidbergvariable{$f_B$ (bootstrap fraction)}{Here $f_B$ is written
...
not a complete bootstrap-current formula. The later Freidberg reconstruction will
...
current-drive burden. That choice will become an explicit Freidberg assumption.}
...
take the exact set just derived and reconstruct, line by line, how Freidberg's 2015
...
The canonical target is Freidberg's Table-1 set,
\[
...
Freidberg parameters or automatically independent design knobs. The two missing
Table-1 hardware parameters are $b$ (blanket-region thickness) and $c$
(TF-magnet thickness). They must earn their place before numerical Freidberg closure
begins.
...
joined the same causal parameter set can the thesis begin Freidberg's numerical
...
\freidbergvariable{$b$ (blanket-region thickness)}{The D--T neutron is neutral, so
...
$\lambda_B$ are supporting transport lengths, not Primary Freidberg parameters. They
...
\freidbergvariable{$c$ (TF-magnet thickness)}{The toroidal-field magnet needs radial
...
% ============================= PAGE 81 ==============================
\newpage
\section*{81. Freidberg's second remedy: raise the field ceiling, not the confinement claim}

The preceding remedy changed the empirical confinement multiplier $H$. Freidberg's
...
not a new primary Freidberg parameter. It changes the primary central field $B_0$ and
...
\model{For Section 6.2 we keep Freidberg's $40\,\mathrm{MW}$ wave-power allocation,
...
remain in Freidberg's conventional design space; only after its closure problem is
...
% ============================= PAGE 87 ==============================
\newpage
\section*{87. Freidberg's third remedy: change the plant scale}
...
For this test Freidberg holds the original plasma and magnet assumptions:
\[
...
point is $P_E=1556\,\mathrm{MW}$ on its displayed grids; Freidberg reports
...
% ============================= PAGE 92 ==============================
\newpage
\section*{92. Freidberg's fourth remedy: lower the permitted wall loading}
...
density, but it makes the reactor physically larger. Freidberg holds
\[
...
is to compare the remedies on one honest ledger before moving beyond Freidberg's
...
% ============================= PAGE 99 ==============================
\newpage
\section*{99. What Freidberg's calculation has rigorously established}
...
\text{bootstrap, and four-limit assumptions}\\[3pt]
\Downarrow\\[3pt]
...
Freidberg's reference calculation then exposed the conventional current closure:
...
Freidberg kink--beta coupling: it changes the geometry of the current that produces
...
% ============================= PAGE 105 ==============================
\newpage
\section*{105. What an ST can plausibly relieve in the Freidberg closure}
...
calculation satisfy the two ratios that failed in Freidberg's conventional baseline,
...
test; it is the separate engineering problem revealed only after the two Freidberg
...
% ============================= PAGE 106 ==============================
\newpage
\section*{106. The ST question is exactly Freidberg's two failed ratios}

Freidberg's conventional reference failed two current-centred tests:
\[
...
Freidberg's bootstrap obstruction only when those two inputs are independently
...
$\sqrt{\epsilon}$. This is the finite-$A$ replacement for Freidberg's
...
necessary: Freidberg's own numerical bootstrap result also required a declared
...
% ============================= PAGE 108 ==============================
\newpage
\section*{108. A published finite-$A$ candidate supplies the kink test and current split}
...
constructing a current balance and testing Freidberg's economic current-drive
constraint.
...
\imagebrief{Freidberg two-constraint closure: published low-A ST operating point}
{Use a two-column, deliberately non-engineering graphic. Left: the conventional
Freidberg reference with $q_*=1.56<2$ and required/calculated bootstrap
...
a fully non-inductive scenario, not as an independently passed Freidberg ratio.
...
than Freidberg's conventional reference threshold: NSTX disruption data show a marked
...
This is an affirmative answer to Freidberg's kink obstruction within the declared
...
% ============================= PAGE 110 ==============================
\newpage
\section*{110. The bootstrap result identifies the price; it does not yet test Freidberg's constraint}
...
an independent Freidberg test must meet:
\begin{equation}
 I_{\rm CD,needed}=(1-f_{\rm BS})I\simeq0.24I
 \simeq(0.24)(10.8\,\mathrm{MA})\simeq2.6\,\mathrm{MA}.
 \label{eq:st-current-drive-needed}
\end{equation}
Here $I\simeq10.8\,\mathrm{MA}$ is paired with $f_{\rm BS}\simeq0.76$ at
normalized density near $0.8$; the full scan's current extrema are not combined
with this selected fraction. The scenario assumes $80\,\mathrm{MW}$ of $0.5\,\mathrm{MeV}$ neutral-beam
heating/current-drive power to obtain this fully non-inductive balance. It therefore
predicts a substantial bootstrap fraction in its equilibrium-informed model and states the associated
external-drive burden; it does not independently establish that a plant can afford
that burden.

The proper Freidberg test remains conditional until an economic/recirculating-power
...
The condition can be priced with published scalars alone. Freidberg's allowance was
$100\,\mathrm{MW}$ of wall electricity for current drive, of which $40\,\mathrm{MW}$
reaches the plasma (a $50\%$ wall-to-klystron and $80\%$ absorption chain); Menard's
FNSF systems analysis assumes a neutral-beam wall-plug efficiency of
$0.4$.\footnote{Freidberg \emph{et al.}, Sec.~4.6; Menard \emph{et al.}, Sec.~2.1.6, where
$\eta_{\rm aux}=0.4$ is described as higher than presently achievable. The arithmetic is reproduced in
\texttt{thesis/code/calculations/st\_drive\_affordability.py}.} Keeping the published
$80\,\mathrm{MW}$ beam state would therefore draw $80/0.4=200\,\mathrm{MW_e}$, twice
that allowance; fitting under it would need a wall-plug efficiency of $0.8$, or, if only
the allowance's $40\,\mathrm{MW}$ were injected, a drive efficacy of about
$2.6\,\mathrm{MA}/40\,\mathrm{MW}\simeq65\,\mathrm{kA/MW}$ at that state. These are
bounds on the published state, not on a redesigned ST: lowering the beam power changes
heating, fast-ion pressure, fusion power, current, and bootstrap fraction together, and
the FNSF point is a $Q_{\rm DT}\simeq2$ neutron-source mission rather than a
$1\,\mathrm{GW_e}$ plant. The affordability question can only be settled on a
power-plant-mission ST scan carrying its own recirculating-power budget.

% ============================= PAGE 111 ==============================
\newpage
\section*{111. Verdict: ST closes the kink obstruction; bootstrap closure has a stated condition}

The published finite-aspect-ratio ST model answers one of Freidberg's two
current-centred obstructions affirmatively:
\[
 {\cal R}_{K,\rm ST}\le0.93<1.
\]
It also predicts a large bootstrap fraction, $f_{\rm BS}\simeq0.76$, and makes
the missing condition quantitative: the operating point needs approximately
$2.6\,\mathrm{MA}$ of independently affordable current drive.

We must not call that second result closure until its current-drive allowance has
been obtained independently. Freidberg's conventional comparison was meaningful
...
promising level. Whether it closes Freidberg's bootstrap obstruction depends on the
separate question $I_{\rm CD,allow}\gtrsim2.6\,\mathrm{MA}$.}
...
% ============================= PAGE 115 ==============================
\newpage
\section*{115. What remains after the ST answers Freidberg's two questions}

The narrowly posed Freidberg result is now established on page 111: the kink
obstruction is closed within the declared finite-$A$ model, and the bootstrap
obstruction has been reduced to an explicit current-drive condition that still awaits
an independent affordability calculation. That result does not settle every physical
or engineering demand placed on a reactor. The following
...
the exhaust problem Freidberg's four ratios intentionally did not close.
\end{enumerate}

The centre-stack calculation begins next. Its task is not to reopen the two
Freidberg ratios; one has been passed and the other has been reduced to a stated
condition. Its task is to measure the exact
...
This fact connects directly to the Freidberg current ledger. A pulsed ST cannot
...
\item Is $\pi R_{\rm cs}^2$ sufficient for the full area ledger in
Eq.~\eqref{eq:centre-stack-area-ledger}?
\item Can the structural allocation withstand the field load within stress and
superconducting strain limits?
...
\[
\begin{gathered}
\boxed{\text{plasma mission}}\\[3pt]
\downarrow\\[3pt]
\boxed{\text{neutron, heat, and tritium functions}}\\[3pt]
\downarrow\\[3pt]
...
% ============================= PAGE 124 ==============================
\newpage
\section*{124. What remains after the Freidberg answer}

The causal argument has reached a useful, deliberately narrow conclusion. The
finite-$A$ analysis in Sections 106--111 passed the stated kink screen and
identified the independent condition still required for bootstrap closure.
...
kink constraint that failed in Freidberg's conventional baseline and reduces the
bootstrap constraint to an explicit, quantified current-drive condition
($I_{\rm CD,allow}\gtrsim2.6\,\mathrm{MA}$, Section 111). Raising $H$ in
Freidberg's conventional family could close the kink and bootstrap tests but
still violated the Troyon bound; it did not close all four screens. It does not establish a theorem
...
scan. It does not relabel that separate engineering question as a failed Freidberg
plasma constraint.
